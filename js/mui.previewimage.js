(function($, window) {
	...
	var itemTemplate = '<div class="mui-slider-item  {{className}}"><div class="mui-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="mui-zoom"></div></div>';
	...
	proto._initZoom = function(zoomWrapperEl, zoomerWidth, zoomerHeight) {
		...
		if (zoomWrapperEl.getAttribute('0')) {
			return;
		}
	...
	proto.close = function(index, group) {
		...
		if (imgEl) {
			...
			if (sTop > window.innerHeight || sLeft > window.innerWidth || sTop < 0 || sLeft < 0) { //out viewport
				...
				imgEl.style.webkitTransitionDuration = '0.0s';
